...
function natEqual(nat1, nat2) {
    var nat1Now = nat1.now();
    var nat2Now = nat2.now();
...
function readBack(context, type, value) {
    ...
    if (typeNow instanceof V.Universe) {
        ...
        return new C.Add1(readBack(context, new V.Nat(), valueNow.smaller));
    }
    else if (typeNow instanceof V.Pi) {
        var y = valueNow instanceof V.Lambda ?
            valueNow.argName : typeNow.argName;
        var freshx = (0, utils_1.fresh)(context, y);
        return new C.Lambda(freshx, readBack((0, context_1.bindFree)(context, freshx, typeNow.argType), typeNow.resultType.valOfClosure(new V.Neutral(typeNow.argType, new N.Variable(freshx))), (0, evaluator_1.doApp)(valueNow, new V.Neutral(typeNow.argType, new N.Variable(freshx)))));
    }
    else if (typeNow instanceof V.Sigma) {
        var car = (0, evaluator_1.doCar)(value);
        var cdr = (0, evaluator_1.doCdr)(value);
        return new C.Cons(readBack(context, typeNow.carType, car), readBack(context, typeNow.cdrType.valOfClosure(car), cdr));
    }
        ...
        return new C.Cons(readBack(context, typeNow.entryType, valueNow.head), readBack(context, new V.List(typeNow.entryType), valueNow.tail));
    }
    else if (typeNow instanceof V.Absurd
        && valueNow instanceof V.Neutral) {
        return new C.The(new C.Absurd(), valueNow.neutral.readBackNeutral(context));
    }
    else if (typeNow instanceof V.Equal
        && valueNow instanceof V.Same) {
        return new C.Same(readBack(context, typeNow.type, valueNow.value));
    }
        ...
        return new C.VecCons(readBack(context, typeNow.entryType, valueNow.head), readBack(context, new V.Vec(typeNow.entryType, typeNow.length.now().smaller), valueNow.tail));
    }
    else if (typeNow instanceof V.Either
        && valueNow instanceof V.Left) {
        return new C.Left(readBack(context, typeNow.leftType, valueNow.value));
    }
    else if (typeNow instanceof V.Either
        && valueNow instanceof V.Right) {
        return new C.Right(readBack(context, typeNow.rightType, valueNow.value));
    }
    else if (valueNow instanceof V.Neutral) {
        return valueNow.neutral.readBackNeutral(context);
    }
    throw new Error("Cannot read back ".concat(valueNow.prettyPrint(), " : ").concat(typeNow.prettyPrint()));
}
