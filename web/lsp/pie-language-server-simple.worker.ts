...
import {
  ...
  DefineTactically,
  schemeParse,
} from "../../src/pie_interpreter/parser/parser";
import {
  DefineDatatypeSource,
  handleDefineDatatype,
} from "../../src/pie_interpreter/typechecker/definedatatype";
import {
  Context,
  initCtx,
  addClaimToContext,
  addDefineToContext,
} from "../../src/pie_interpreter/utils/context";
...
function processDeclaration(
  ...
  try {
    if (decl instanceof Claim) {
      ...
      newContext = processDefineTacticallyDeclaration(
        decl,
        context,
        diagnostics,
      );
    } else if (decl instanceof DefineDatatypeSource) {
      const result = processDefineDatatypeDeclaration(
        decl,
        context,
        renaming,
        diagnostics,
...
function processDefineDeclaration(
  define: Definition,
  context: Context,
  diagnostics: Diagnostic[],
): Context {
...
function processDefineTacticallyDeclaration(
  ...
  try {
    ...
    // Start the proof
    const startResult = proofManager.startProof(
      defineTactically.name,
      context,
      defineTactically.location,
    );
...
const PIE_COMPLETIONS = [
  ...
  { label: "Pi", kind: "TypeParameter", detail: "Dependent function type" },
  {
    label: "Π",
    kind: "TypeParameter",
    detail: "Dependent function type (Unicode)",
  },
  { label: "Sigma", kind: "TypeParameter", detail: "Dependent pair type" },
  {
    label: "Σ",
    kind: "TypeParameter",
    detail: "Dependent pair type (Unicode)",
  },
  ...
  // Equality functions
  {
    label: "replace",
    kind: "Function",
    detail: "Substitution of equals for equals",
  },
  ...
  {
    label: "define-tactically",
    kind: "Keyword",
    detail: "Define using tactics",
  },
];
...
function extractUserDefinedSymbols(source: string): {
  ...
  const defineTacticallyPattern =
    /\(define-tactically\s+([a-zA-Z][a-zA-Z0-9\-_!?*+=<>]*)/;
...
function getWordAndRange(
  source: string,
  line: number,
  column: number,
): { word: string; start: number; end: number; cursorOffset: number } | null {
  const lines = source.split("\n");
...
self.onmessage = (event: MessageEvent) => {
  ...
  if (type === "validate") {
    ...
    try {
      ...
      const prefix = wordInfo
        ? wordInfo.word.substring(0, wordInfo.cursorOffset)
        : "";
    ...
    try {
      ...
      if (definition) {
        const hoverInfo = {
          ...
          details: definition.typeInfo
            ? `Type: ${definition.typeInfo}`
            : undefined,
          examples: undefined,
        };
        self.postMessage({
