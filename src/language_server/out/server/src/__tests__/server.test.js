...
async function validateTextDocument(textDocument, maxProblems = 1000) {
...
describe('LSP Server Tests', () => {
    describe('Diagnostic Tests', () => {
        test('should detect uppercase words', async () => {
            const document = vscode_languageserver_textdocument_1.TextDocument.create('file:///test.txt', 'plaintext', 1, 'This has UPPERCASE and lowercase words');
            const diagnostics = await validateTextDocument(document);
            expect(diagnostics).toHaveLength(1);
            expect(diagnostics[0].message).toBe('UPPERCASE is all uppercase.');
            expect(diagnostics[0].severity).toBe(node_1.DiagnosticSeverity.Warning);
        });
        test('should detect multiple uppercase words', async () => {
            const document = vscode_languageserver_textdocument_1.TextDocument.create('file:///test.txt', 'plaintext', 1, 'HELLO WORLD and TEST');
            const diagnostics = await validateTextDocument(document);
        ...
        test('should not detect single uppercase letters', async () => {
            const document = vscode_languageserver_textdocument_1.TextDocument.create('file:///test.txt', 'plaintext', 1, 'A B C are single letters');
            const diagnostics = await validateTextDocument(document);
            expect(diagnostics).toHaveLength(0);
        });
        test('should not detect lowercase words', async () => {
            const document = vscode_languageserver_textdocument_1.TextDocument.create('file:///test.txt', 'plaintext', 1, 'these are all lowercase words');
            const diagnostics = await validateTextDocument(document);
            expect(diagnostics).toHaveLength(0);
        });
        test('should respect max problems limit', async () => {
            const document = vscode_languageserver_textdocument_1.TextDocument.create('file:///test.txt', 'plaintext', 1, 'ONE TWO THREE FOUR FIVE SIX');
            const diagnostics = await validateTextDocument(document, 3);
            expect(diagnostics).toHaveLength(3);
        });
        test('should handle Pie language constructs', async () => {
            const document = vscode_languageserver_textdocument_1.TextDocument.create('file:///test.pie', 'pie', 1, '(define CONSTANT 42)\n(lambda (X Y) (+ X Y))');
            const diagnostics = await validateTextDocument(document);
