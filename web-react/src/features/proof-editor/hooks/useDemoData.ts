...
export function useDemoData() {
  ...
  useEffect(() => {
    ...
    connectNodes(introTacticId, subGoalId, {
      kind: "tactic-to-goal",
      outputIndex: 0,
    });
    ...
    connectNodes(elimTacticId, baseGoalId, {
      kind: "tactic-to-goal",
      outputIndex: 0,
    });
    ...
    const indGoalId = addGoalNode(
      {
        kind: "goal",
        goalType:
          "(Π ((k Nat) (ih (= Nat (+ k zero) k))) (= Nat (+ (add1 k) zero) (add1 k)))",
    ...
    connectNodes(elimTacticId, indGoalId, {
      kind: "tactic-to-goal",
      outputIndex: 1,
    });
