...
function alphaEquivAux(lvl, b1, b2, e1, e2) {
    if (e1 instanceof C.VarName && e2 instanceof C.VarName) {
        var n1 = e1.name;
        var n2 = e2.name;
        if ((0, utils_1.isVarName)(n1) && (0, utils_1.isVarName)(n2)) {
            var xBinding = findBinding(n1, b1);
            var yBinding = findBinding(n2, b2);
            // both bound
        ...
        return alphaEquivAux(lvl + 1, bind(b1, e1.param, lvl), bind(b2, e2.param, lvl), e1.body, e2.body);
    }
    ...
    else if ((e1 instanceof C.Universe && e2 instanceof C.Universe) ||
