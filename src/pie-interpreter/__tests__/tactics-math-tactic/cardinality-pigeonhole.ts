...
(claim len-empty (= Nat (length Nat (the (List Nat) nil)) 0))
(define-tactically len-empty
  ((exact (same 0))))
`;
    ...
    it("singleton list has length 1", () => {
      const str = `${lengthPreamble}
(claim len-single (= Nat (length Nat (:: 5 nil)) 1))
(define-tactically len-single
  ((exact (same 1))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("two-element list has length 2", () => {
      const str = `${lengthPreamble}
(claim len-two (= Nat (length Nat (:: 1 (:: 2 nil))) 2))
(define-tactically len-two
  ((exact (same 2))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("three-element list has length 3", () => {
      const str = `${lengthPreamble}
(claim len-three (= Nat (length Nat (:: 10 (:: 20 (:: 30 nil)))) 3))
(define-tactically len-three
  ((exact (same 3))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("five-element list has length 5", () => {
      const str = `${lengthPreamble}
(claim len-five (= Nat (length Nat (:: 1 (:: 2 (:: 3 (:: 4 (:: 5 nil)))))) 5))
(define-tactically len-five
  ((exact (same 5))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("Vec of length 0: vecnil", () => {
      const str = `
(claim v0 (Vec Nat 0))
(define-tactically v0
  ((exact (the (Vec Nat 0) vecnil))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("Vec of length 1", () => {
      const str = `
(claim v1 (Vec Nat 1))
(define-tactically v1
  ((exact (the (Vec Nat 1) (vec:: 42 vecnil)))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("Vec of length 3", () => {
      const str = `
(claim v3 (Vec Nat 3))
(define-tactically v3
  ((exact (the (Vec Nat 3) (vec:: 1 (vec:: 2 (vec:: 3 vecnil)))))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("length of Atom list", () => {
      const str = `${lengthPreamble}
(claim len-atom (= Nat (length Atom (:: 'a (:: 'b (:: 'c nil)))) 3))
(define-tactically len-atom
  ((exact (same 3))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(claim len-append-1-1 (= Nat (length Nat (append Nat (:: 1 nil) (:: 2 nil))) 2))
(define-tactically len-append-1-1
  ((exact (same 2))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(claim append (Π ((E U)) (→ (List E) (List E) (List E))))
(define append (λ (E l1 l2) (rec-List l1 l2 (λ (e es acc) (:: e acc)))))
(claim len-append-2-3 (= Nat (length Nat (append Nat (:: 1 (:: 2 nil)) (:: 3 (:: 4 (:: 5 nil))))) 5))
(define-tactically len-append-2-3
  ((exact (same 5))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(claim append (Π ((E U)) (→ (List E) (List E) (List E))))
(define append (λ (E l1 l2) (rec-List l1 l2 (λ (e es acc) (:: e acc)))))
(claim len-append-nil (= Nat (length Nat (append Nat (the (List Nat) nil) (:: 1 (:: 2 nil)))) 2))
(define-tactically len-append-nil
  ((exact (same 2))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(claim len-map (= Nat (length Atom (map Nat Atom (λ (n) 'x) (:: 1 (:: 2 (:: 3 nil))))) 3))
(define-tactically len-map
  ((exact (same 3))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(claim l1 (List Nat))
(define l1 (:: 1 (:: 2 (:: 3 nil))))
(claim len-l1 (= Nat (length Nat l1) 3))
(define-tactically len-l1
  ((exact (same 3))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(claim outer-len-check (= Nat outer-len 2))
(define-tactically outer-len-check
  ((exact (same 2))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("length of 4-element Atom list", () => {
      const str = `${lengthPreamble}
(claim len-atom-4 (= Nat (length Atom (:: 'a (:: 'b (:: 'c (:: 'd nil))))) 4))
(define-tactically len-atom-4
  ((exact (same 4))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
`;
    ...
    it("adding element to Vec increases size", () => {
      const str = `
(claim v2 (Vec Nat 2))
(define v2 (the (Vec Nat 2) (vec:: 1 (vec:: 2 vecnil))))
(claim v3 (Vec Nat 3))
(define-tactically v3
  ((exact (the (Vec Nat 3) (vec:: 0 v2)))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(define-tactically triv-to-nat
  ((intro t)
   (exact 0)))
(claim triv-to-nat-check (= Nat (triv-to-nat sole) 0))
(define-tactically triv-to-nat-check
  ((exact (same 0))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(define-tactically bool-to-nat
  ((exact (λ (b) (ind-Either b (λ (x) Nat) (λ (l) 0) (λ (r) 1))))))
(claim btn-left (= Nat (bool-to-nat (left sole)) 0))
(define-tactically btn-left
  ((exact (same 0))))
(claim btn-right (= Nat (bool-to-nat (right sole)) 1))
(define-tactically btn-right
  ((exact (same 1))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(claim rt-0 (= Nat (to-nat (from-nat 0)) 0))
(define-tactically rt-0
  ((exact (same 0))))
(claim rt-1 (= Nat (to-nat (from-nat 1)) 1))
(define-tactically rt-1
  ((exact (same 1))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(define-tactically pair-vec
  ((exact (the (Vec Nat 2) (vec:: 10 (vec:: 20 vecnil))))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
