...
`;
    ...
    it("computes append of nil and (:: 1 nil)", () => {
      const str = `${listPreamble}
(length Nat (append Nat (the (List Nat) nil) (:: 1 nil)))
`;
      const output = evaluatePie(str);
      expect(output).toContain("1: Nat");
    });
...
`;
    ...
    it("computes map add1 over (:: 1 (:: 2 nil)) gives length 2", () => {
      const str = `${listPreamble}
(length Nat (map Nat Nat (+ 1) (:: 1 (:: 2 nil))))
`;
      const output = evaluatePie(str);
      expect(output).toContain("2: Nat");
    });
...
(define-tactically tac-map
  ((intro A)
   ...
   (then
     ...
     (intro ih)
     (exact (:: (f x) ih)))))
(length Nat (tac-map Nat Nat (+ 1) (:: 1 (:: 2 (:: 3 nil)))))
`;
      const output = evaluatePie(str);
      expect(output).toContain("3: Nat");
    });
...
(length Nat (reverse Nat (:: 1 (:: 2 (:: 3 nil)))))
`;
      const output = evaluatePie(str);
      expect(output).toContain("3: Nat");
    });
...
(define-tactically list-to-nat
  ((intro xs)
   ...
   (then
     ...
     (intro ih)
     (exact (+ x ih)))))
(list-to-nat (:: 10 (:: 20 (:: 30 nil))))
`;
      const output = evaluatePie(str);
      expect(output).toContain("60: Nat");
    });
...
(length Nat (keep-zeros (:: 0 (:: 1 (:: 0 (:: 3 nil))))))
`;
      const output = evaluatePie(str);
      expect(output).toContain("2: Nat");
    });
