...
export const TacticNode = memo(function TacticNode({
  ...
  return (
    ...
    >
      ...
      {data.status === "incomplete" &&
        needsContextInput &&
        !data.parameters.targetContextId && (
      ...
      {needsContextInput && data.parameters.targetContextId && (
        <div className="mt-1 flex items-center gap-1 text-xs text-blue-600">
          <span className="rounded bg-blue-100 px-1 py-0.5">
            target:{" "}
            {data.parameters.variableName || data.parameters.targetContextId}
          </span>
        </div>
      )}
      ...
      {data.status !== "incomplete" && data.parameters.expression && (
        <div
          className="mt-1 text-xs text-gray-600 font-mono truncate"
          title={data.parameters.expression}
        >
