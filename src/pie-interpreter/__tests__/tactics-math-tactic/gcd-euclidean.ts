...
`;
    ...
    it("computes monus(7,3) = 4", () => {
      const str = `${arithPreamble}
${monusDef}
(claim monus-7-3 (= Nat (monus 7 3) 4))
(define-tactically monus-7-3
  ((exact (same 4))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("computes monus(0,5) = 0", () => {
      const str = `${arithPreamble}
${monusDef}
(claim monus-0-5 (= Nat (monus 0 5) 0))
(define-tactically monus-0-5
  ((exact (same 0))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("computes monus(10,10) = 0", () => {
      const str = `${arithPreamble}
${monusDef}
(claim monus-10-10 (= Nat (monus 10 10) 0))
(define-tactically monus-10-10
  ((exact (same 0))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("computes monus(6,2) = 4", () => {
      const str = `${arithPreamble}
${monusDef}
(claim monus-6-2 (= Nat (monus 6 2) 4))
(define-tactically monus-6-2
  ((exact (same 4))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("computes monus(1,0) = 1", () => {
      const str = `${arithPreamble}
${monusDef}
(claim monus-1-0 (= Nat (monus 1 0) 1))
(define-tactically monus-1-0
  ((exact (same 1))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("computes monus(4,4) = 0", () => {
      const str = `${arithPreamble}
${monusDef}
(claim monus-4-4 (= Nat (monus 4 4) 0))
(define-tactically monus-4-4
  ((exact (same 0))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("computes monus(8,3) = 5", () => {
      const str = `${arithPreamble}
${monusDef}
(claim monus-8-3 (= Nat (monus 8 3) 5))
(define-tactically monus-8-3
  ((exact (same 5))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("computes monus(9,7) = 2", () => {
      const str = `${arithPreamble}
${monusDef}
(claim monus-9-7 (= Nat (monus 9 7) 2))
(define-tactically monus-9-7
  ((exact (same 2))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("computes monus(2,0) = 2", () => {
      const str = `${arithPreamble}
${monusDef}
(claim monus-2-0 (= Nat (monus 2 0) 2))
(define-tactically monus-2-0
  ((exact (same 2))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
    ...
    it("computes monus(100,99) = 1", () => {
      const str = `${arithPreamble}
${monusDef}
(claim monus-100-99 (= Nat (monus 100 99) 1))
(define-tactically monus-100-99
  ((exact (same 1))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(define-tactically gcd-6-4
  ((exists 2 d)
   ...
   (then (exact (same 6)))
   (then (exact (same 4)))))
(claim gcd-6-4-result (= Nat (car gcd-6-4) 2))
(define-tactically gcd-6-4-result
  ((exact (same 2))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(define-tactically gcd-9-6
  ((exists 3 d)
   ...
   (then (exact (same 9)))
   (then (exact (same 6)))))
(claim gcd-9-6-result (= Nat (car gcd-9-6) 3))
(define-tactically gcd-9-6-result
  ((exact (same 3))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(define-tactically gcd-8-12
  ((exists 4 d)
   ...
   (then (exact (same 8)))
   (then (exact (same 12)))))
(claim gcd-8-12-result (= Nat (car gcd-8-12) 4))
(define-tactically gcd-8-12-result
  ((exact (same 4))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(define-tactically gcd-15-10
  ((exists 5 d)
   ...
   (then (exact (same 15)))
   (then (exact (same 10)))))
(claim gcd-15-10-result (= Nat (car gcd-15-10) 5))
(define-tactically gcd-15-10-result
  ((exact (same 5))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(define-tactically gcd-14-21
  ((exists 7 d)
   ...
   (then (exact (same 14)))
   (then (exact (same 21)))))
(claim gcd-14-21-result (= Nat (car gcd-14-21) 7))
(define-tactically gcd-14-21-result
  ((exact (same 7))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(define-tactically gcd-18-12
  ((exists 6 d)
   ...
   (then (exact (same 18)))
   (then (exact (same 12)))))
(claim gcd-18-12-result (= Nat (car gcd-18-12) 6))
(define-tactically gcd-18-12-result
  ((exact (same 6))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(define-tactically gcd-20-15
  ((exists 5 d)
   ...
   (then (exact (same 20)))
   (then (exact (same 15)))))
(claim gcd-20-15-result (= Nat (car gcd-20-15) 5))
(define-tactically gcd-20-15-result
  ((exact (same 5))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
...
(define-tactically gcd-16-24
  ((exists 8 d)
   ...
   (then (exact (same 16)))
   (then (exact (same 24)))))
(claim gcd-16-24-result (= Nat (car gcd-16-24) 8))
(define-tactically gcd-16-24-result
  ((exact (same 8))))
`;
      expect(() => evaluatePie(str)).not.toThrow();
    });
